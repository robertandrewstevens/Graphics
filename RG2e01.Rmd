...
https://www.stat.auckland.ac.nz/%7Epaul/RG2e/examples-infographic.R
...
pushViewport(viewport(width = unit(1, "snpc"), height = unit(1, "snpc"),
                      gp = gpar(cex = 1.2)))
grid.polygon(x, y, gp = gpar(col = NA, fill = rgb(0.67, 0, 0.11, 0.7)))
label1 <- textGrob("UNACCOUNTED\nFOR",
                   unit(0.2, "npc") - unit(2, "mm"),
                   unit(0.6, "npc") + unit(2, "mm"),
                   gp = gpar(cex = 1.4, fontface = "bold"),
                   just = c("right", "bottom"))
grid.rect(0.2, 0.6, just = c("right", "bottom"),
          width = grobWidth(label1) + unit(4, "mm"),
          height = grobHeight(label1) + unit(4, "mm"),
          gp = gpar(col = NA, fill = rgb(1, 1, 1, 0.5)))
grid.draw(label1)
label2 <- textGrob("ACCOUNTED\nFOR", 
                   unit(0.8, "npc") + unit(2, "mm"),
                   unit(0.6, "npc") + unit(2, "mm"),
                   gp = gpar(cex = 1.4, fontface = "bold"),
                   just = c("left", "bottom"))
grid.rect(0.8, 0.6, just = c("left", "bottom"),
          width = grobWidth(label2) + unit(4, "mm"),
          height = grobHeight(label2) + unit(4, "mm"),
          gp = gpar(col = NA, fill = rgb(1, 1, 1, 0.5)))
grid.draw(label2)
grid.segments(c(0.2, 0.8), 0.6,
              c(0.3, 0.7), 0.5,
...
https://www.stat.auckland.ac.nz/%7Epaul/RG2e/organisation-graphicslevels.R

```{r comment=NA}
library(graph) # package ‘graph’ is not available (for R version 3.2.2)
library(Rgraphviz) # package ‘Rgraphviz’ is not available (for R version 3.2.2)
...
nodeData(ragraph, 
         c("grDevices", "graphics", "grid", "lattice", "ggplot2"), 
         "style") <- "filled"
nodeData(ragraph, 
         c("grDevices", "graphics", "grid", "lattice", "ggplot2"), 
         "fillcolor") <- "gray70"
